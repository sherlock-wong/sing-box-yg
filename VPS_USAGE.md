...
自定义普通 TLS 域名时，安装不会直接把域名配到临时自签证书上：会先扫描证书库和已有 ACME 证书。找到覆盖该域名的受信证书会直接写入并绑定；找不到时会在首次安装中让你选择运行 ACME、导入已有证书，或明确选择暂时使用匹配的固定自签证书。ACME/导入的证书会同时写入证书库、绑定给本次安装的普通 TLS 协议，避免再修改一次。
