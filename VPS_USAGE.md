...
在 `vm` 主菜单选择“证书管理”可查看证书及 Hysteria2/AnyTLS 的绑定、导入已有 PEM 证书/私钥、创建固定自签证书、运行锁定版 ACME 域名证书申请，或立即同步受管来源。导入时可选择持续跟踪原始证书文件；启用后，systemd 每 6 小时执行一次同步。申请 ACME 时，脚本会在当前终端交互；旧脚本临时输出的 `/root/ygkkkca/` 只作为导入来源，成功后 VPNM 验证 SAN、公私钥与有效期，再保存为 `/etc/vps-net-manager/certs/<证书ID>/fullchain.pem` 和 `privkey.pem`。
