...
在 `vm` 主菜单选择“证书管理”可查看证书及 Hysteria2/AnyTLS 的绑定、导入已有 PEM 证书/私钥、创建固定自签证书、运行官方锁定版 `acme.sh` 域名证书申请，或立即同步受管来源。“管理证书来源路径”可为已入库证书更换外部 PEM 证书/私钥路径，或停止追踪来源；服务实际使用的归档路径固定在 `/etc/vps-net-manager/certs/<证书ID>/`。导入时可选择持续跟踪原始证书文件；申请 ACME 时可选择 Cloudflare DNS API 或独立 HTTP-01 验证，VPNM 不会停止其他服务。VPNM 每 6 小时先运行 ACME 续期检查，再同步已续期的证书。申请输出固定在 `/etc/vps-net-manager/acme/<证书ID>/`，验证 SAN、公私钥与有效期后归档为 `/etc/vps-net-manager/certs/<证书ID>/fullchain.pem` 和 `privkey.pem`。
